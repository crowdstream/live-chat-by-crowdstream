...
class LiveChatByCrowdstream_OptionsManager {
    ...
    public function settingsPage() {
        if (!current_user_can('manage_options')) {
            wp_die(__('You do not have sufficient permissions to access this page.', 'live-chat-by-crowdstream'));
        }
        ...
        ?>
        <div class="wrap">
            <h2><?php _e('System Settings', 'live-chat-by-crowdstream'); ?></h2>
            <table cellspacing="1" cellpadding="2"><tbody>
            <tr><td><?php _e('System', 'live-chat-by-crowdstream'); ?></td><td><?php echo php_uname(); ?></td></tr>
            <tr><td><?php _e('PHP Version', 'live-chat-by-crowdstream'); ?></td>
                <td><?php echo phpversion(); ?>
                <?php
                if (version_compare('5.2', phpversion()) > 0) {
                    echo '&nbsp;&nbsp;&nbsp;<span style="background-color: #ffcc00;">';
                    _e('(WARNING: This plugin may not work properly with versions earlier than PHP 5.2)', 'live-chat-by-crowdstream');
                    echo '</span>';
                }
                ?>
                </td>
            </tr>
            <tr><td><?php _e('MySQL Version', 'live-chat-by-crowdstream'); ?></td>
                <td><?php echo $this->getMySqlVersion() ?>
                    <?php
                    echo '&nbsp;&nbsp;&nbsp;<span style="background-color: #ffcc00;">';
                    if (version_compare('5.0', $this->getMySqlVersion()) > 0) {
                        _e('(WARNING: This plugin may not work properly with versions earlier than MySQL 5.0)', 'live-chat-by-crowdstream');
                    }
                    echo '</span>';
                    ?>
                </td>
            ...
            <h2><?php echo $this->getPluginDisplayName(); echo ' '; _e('Settings', 'live-chat-by-crowdstream'); ?></h2>
            ...
            <?php settings_fields($settingsGroup); ?>
                ...
                <p class="submit">
                    <input type="submit" class="button-primary"
                           value="<?php _e('Save Changes', 'live-chat-by-crowdstream') ?>"/>
                </p>
            </form>
        </div>
        <?php
    ...
    protected function getOptionValueI18nString($optionValue) {
        switch ($optionValue) {
            case 'true':
                return __('true', 'live-chat-by-crowdstream');
            case 'false':
                return __('false', 'live-chat-by-crowdstream');

            case 'Administrator':
                return __('Administrator', 'live-chat-by-crowdstream');
            case 'Editor':
                return __('Editor', 'live-chat-by-crowdstream');
            case 'Author':
                return __('Author', 'live-chat-by-crowdstream');
            case 'Contributor':
                return __('Contributor', 'live-chat-by-crowdstream');
            case 'Subscriber':
                return __('Subscriber', 'live-chat-by-crowdstream');
            case 'Anyone':
                return __('Anyone', 'live-chat-by-crowdstream');
        }
        return $optionValue;
    }
